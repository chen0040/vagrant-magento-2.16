...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: "[ -d /home/vagrant/.composer ] || mkdir /home/vagrant/.composer"
  config.vm.provision "shell", inline: "[ -d /root/.composer ] || mkdir /root/.composer"
  config.vm.provision "shell", inline: "echo '{\"http-basic\": {\"repo.magento.com\": {\"username\": \"#{vagrantConfig ['http_basic']['repo_magento_com']['username']}\",\"password\": \"#{vagrantConfig['http_basic']['repo_magento_com']['password']}\"}}}' > /home/vagrant/.composer/auth.json"
  ...
  config.vm.provision "shell", inline: "composer create-project --allow-root --ignore-platform-reqs --repository-url=https://repo.magento.com/ magento/project-community-edition /var/www/html/"
